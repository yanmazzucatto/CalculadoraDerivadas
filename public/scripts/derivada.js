...
function derivada(expr) {
  ...
  try {
    ...
    const strDeriv1 = derivada1simp.toString().trim()  // ex: "0" para constantes
    ...
    //  ───────────────────────────────────────────────────────────────────
    //  Se f′(x) for exatamente "0", não faz sentido varrer todo o intervalo
    //  e achar "pontos críticos numéricos". Basta informar que não há extremos
    //  (ou "infinitos pontos triviais").
    //  ───────────────────────────────────────────────────────────────────
    if (strDeriv1 === "0") {
      // f'(x)=0 para todo x → classe de polinômio constante/linear
      html += `<em>f′(x) ≡ 0 para todo x → sem pontos extremos distintos.</em>`
      areaResposta.innerHTML = html
      return
    }

    // ───  Caso padrão (quando f′(x) ≠ 0 identicamente), fazemos a varredura  ───
    const f1 = x => {
